    ...
    <section className="mx-auto max-w-6xl px-4 py-12 sm:py-20">
      <div className="grid gap-6 sm:grid-cols-2">
        {groups.map((group, idx) => (
          ...
          >
            <Card className="group rounded-2xl border-border/80 p-6 transition-colors hover:border-foreground/30">
              ...
              <ul className="mt-4 grid grid-cols-2 gap-2 text-sm text-muted-foreground sm:grid-cols-3">
                {group.items.map((item) => (
                  <li
                    key={item}
                    className="rounded-full border border-border/70 px-3 py-2 transition-colors group-hover:border-foreground/30 hover:bg-secondary"
                  >
                    {item}
                  </li>
                ))}
